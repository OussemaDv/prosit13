...
public class Level2 {
    ...
    public static void main(String[] args) {
        ...
        long nbr = teachers.stream().filter(teacher -> teacher.getName().startsWith("n")).count();
        ...
        double average = teachers.stream().filter(teacher -> teacher.getName().startsWith("n")).mapToInt(teacher->teacher.getSalary()).average().getAsDouble();
        ...
        /*First way*/
        teachers.stream().filter(teacher -> teacher.getName().startsWith("g")).limit(1);
        ;
        /*Second way*/
        teachers.stream().filter(teacher -> teacher.getName().startsWith("g")).findFirst();

        /* TO DO 7: Afficher le deuxième enseignant dont le nom commence avec s */
        teachers.stream().filter(teacher -> teacher.getName().startsWith("g")).skip(1).findFirst();
